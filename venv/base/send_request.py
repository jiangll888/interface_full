...
class SendRequest:
    ...
    def send_put(self,url,data,file=None,param=None,header=None,cookie=None):
        # 解决请求中有中文之后截断的问题
        if data:
            data = data.encode(encoding="utf-8")
            if header and settings.HEADER_URLENCODE in header[settings.HEADER_TYPE]:
            ...
            data = {}
        if cookie and header:
            res = requests.put(url=url, data=data, json=data, files=file, params=param, headers=header, cookies=cookie)
        elif cookie and not header:
            res = requests.put(url=url, data=data, json=data, files=file, params=param, cookies=cookie)
        elif not cookie and header:
            res = requests.put(url=url, data=data, json=data, files=file, params=param, headers=header)
        else:
            res = requests.put(url=url, data=data, json=data, files=file, params=param)
        return res
    ...
    def send_request(self,method,url,data=None,file=None,param=None,header=None,cookie=None):
        if method.lower() == "get":
            ...
            res = self.send_put(url=url,data=data,file=file,param=param,header=header,cookie=cookie)
        else:
